...
public class Tracker extends SubsystemBase {
    ...
    public Tracker() {
        ...
        RobotModeTriggers.teleop().onTrue(Commands.parallel(
            transition(),
            printMatchTime().asProxy(),
            signalEndOfShift(true)
        ));

        new Trigger(() -> matchTime() < 130).onTrue(Commands.parallel(
            autoLoserShift(),
            signalEndOfShift(false)
        ));

        new Trigger(() -> matchTime() < 105).onTrue(Commands.parallel(
            autoWinnerShift(),
            signalEndOfShift(false)
        ));

        new Trigger(() -> matchTime() < 80).onTrue(Commands.parallel(
            autoLoserShift(),
            signalEndOfShift(false)
        ));

        new Trigger(() -> matchTime() < 55).onTrue(Commands.parallel(
            autoWinnerShift(),
            signalEndOfShift(false)
        ));
    ...
    private static Command shiftCountdown(String key) {
        return Commands.waitSeconds(shiftTime).deadlineFor(
            Commands.startRun(
                () -> shiftTimer.restart(),
    ...
    private Command signalEndOfShift(boolean immediately) {
        Command blink = Commands.waitSeconds(10.0).deadlineFor(Commands.run(
            () -> {
                if((int) matchTime() % 2 == 0)
                    SmartDashboard.putBoolean("Transition", true);
                else
                    SmartDashboard.putBoolean("Transition", false);
            }
        ));
    ...
    private static Command transition() {
        return Commands.runOnce(() -> {
            ...
            if(whoWon == "B")
                SmartDashboard.putBoolean("Blue Auto Won", true);
            else if(whoWon == "R")
                SmartDashboard.putBoolean("Red Auto Won", true);
        });
    }
